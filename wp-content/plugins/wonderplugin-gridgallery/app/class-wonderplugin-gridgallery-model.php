...
class WonderPlugin_Gridgallery_Model {
	...
	function generate_lightbox_code($id, $data, $slide, $isbutton) {
		...
		$isinlinevideo = ($slide->type >= 1 && $slide->type <= 4 && ($slide->playvideoinline || $slide->loadvideoinline) && !$isbutton);
	...
	function generate_body_code($id, $has_wrapper, $datatags = null) {
		...
		{			
			...
			$boolOptions = array('random', 'shownavigation', 'nohoverontouchscreen', 'hoverzoomin', 'hoverzoominimageonly', 'hoverzoominimagecenter', 'hoverfade', 'responsive', 'mediumscreen', 'smallscreen', 'showtitle', 'showtexttitle', 'showtextdescription', 'showtextbutton', 'overlaylink','donotaddtext', 'lightboxnogroup', 'lightboxcategorygroup', 
